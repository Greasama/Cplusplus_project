...
void Chess::init()
{
    ...
    //棋盘大小
    loadimage(0, "res/棋盘2.jpg");//显示棋盘图片
    mciSendString("play res/start.wav", 0, 0, 0);//播放开始提示音
    loadimage(&chessBlackImg, "res/black.png", chessSize, chessSize, true);//加载黑棋素材，可等比例缩小
    loadimage(&chessWhiteImg, "res/white.png", chessSize, chessSize, true);//加载白棋素材，可等比例缩小
    for (int i = 0; i < chessMap.size(); i++)//棋盘清理
    {
        for (int j = 0; j < gradeSize; j++)
        {
            chessMap[i][j] = 0;
